...
bool BasicSfM::incrementalReconstruction( int seed_pair_idx0, int seed_pair_idx1 )
{
  ...
  if (inliers_E <= inliers_H || inliers_E < 10)
    return false;

  double score = cv::recoverPose(E, points0, points1, intrinsics_matrix, init_r_mat, init_t_vec, inlier_mask_E);

  // Check sideward motion
  if (std::abs(init_t_vec.at<double>(2)) > std::abs(init_t_vec.at<double>(0)) + std::abs(init_t_vec.at<double>(1)))
    return false;
  ...
  for(int iter = 1; iter < num_cam_poses_ - 1; iter++ )
  {
        ...
        for( auto const& co_iter : cam_observation_[cam_idx] )
        {
              ...
              cam_observation_[new_cam_pose_idx].find(pt_idx ) != cam_observation_[new_cam_pose_idx].end() )
          {
            ...
            //////////////////////////// Code to be completed (4/7) /////////////////////////////////
                       // Triangulate the 3D point with index pt_idx using the two poses
            cv::Mat R0, R1;
            cv::Rodrigues(cv::Mat(3, 1, CV_64F, cam0_data), R0);
            cv::Rodrigues(cv::Mat(3, 1, CV_64F, cam1_data), R1);
            ...
            cv::Mat P0 = cv::Mat::zeros(3, 4, CV_64F), P1 = cv::Mat::zeros(3, 4, CV_64F);
            R0.copyTo(P0(cv::Rect(0, 0, 3, 3)));
            t0.copyTo(P0(cv::Rect(3, 0, 1, 3)));
            ...
            cv::triangulatePoints(P0, P1, points0, points1, hpoints4D);
            cv::Mat pt3d_h = hpoints4D.col(0);
            cv::Mat pt3d = pt3d_h.rowRange(0, 3) / pt3d_h.at<double>(3);

            double *pt = pointBlockPtr(pt_idx);
            pt[0] = pt3d.at<double>(0);
            pt[1] = pt3d.at<double>(1);
            pt[2] = pt3d.at<double>(2);
    ...
    if (n_new_pts < 5) {
  std::cout << "Too much few new points added in this iteration. Reconstruction might be diverging." << std::endl;
  return false;
  }
    ...
    for( int i_obs = 0; i_obs < num_observations_; i_obs++ )
    {
      ...
      if( cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0 )
      {
		...
		problem.AddResidualBlock(
  			cost_function,
  			loss_function,
 		 parameters_.data() + camera_block_size_ * cam_pose_index_[i_obs],
		 parameters_.data() + camera_block_size_ * num_cam_poses_ + point_block_size_ * point_index_[i_obs]
);
